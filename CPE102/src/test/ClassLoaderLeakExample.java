 ...
 *
 * <p>
 * To see it in action, copy this file to a temp directory somewhere, and then
 * run:
 * 
 * <pre>
 * {@code
 ...
 *
 * <p>
 * And watch the memory grow! On my system, using JDK 1.8.0_25, I start getting
 * OutofMemoryErrors within just a few seconds.
 *
 * <p>
 * This class is implemented using some Java 8 features, mainly for convenience
 * in doing I/O. The same basic mechanism works in any version of Java since
 * 1.2.
 */
public final class ClassLoaderLeakExample {
	...
	/**
	 * Implementation of the thread. It just calls {@link #loadAndDiscard()} in
	 * a loop.
	 */
	...
	/**
	 * A simple ClassLoader implementation that is only able to load one class,
	 * the LoadedInChildClassLoader class. We have to jump through some hoops
	 * here because we explicitly want to ensure we get a new class each time
	 * (instead of reusing the class loaded by the system class loader). If this
	 * child class were in a JAR file that wasn't part of the system classpath,
	 * we wouldn't need this mechanism.
	 */
	 ...
	 * and then discards any reference to them. Theoretically, there should be
	 * no GC impact, since no references can escape this method! But in practice
	 * this will leak memory like a sieve.
	 */
